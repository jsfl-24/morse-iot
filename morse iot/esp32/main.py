    ...
    try:
        ...
        if not file_exists(filename):
            ...
            try:
                ...
                print("❌ Error reading userdata.json:", str(e))
                users = {}
        ...
        # ✅ Update score (ensure it saves correctly)
        users[username]["memory_mode"] = max(users[username]["memory_mode"], score)

        # ✅ Save back to file (with explicit writing)
        try:
            with open(filename, "w") as f:
                ujson.dump(users, f)
                f.flush()  # Ensure data is actually written
        except OSError as e:
            print("❌ Error writing to userdata.json:", str(e))
            return Response(ujson.dumps({"success": False, "error": "Failed to save data"}), headers={"Content-Type": "application/json"})

        print(f"✅ Successfully saved score for {username}: {users[username]['memory_mode']}")  # Debugging log
...
# ajax, gpio...from microdot import Microdot, Responsefrom microdot import Microdot, Response
