...
// Function to fetch Morse input from ESP32
function fetchMorseInput() {
    fetch("/api/get-morse-input") // Request Morse input from the server
        .then(response => response.json())
        .then(data => {
            if (data.morse) {
                document.getElementById("morseInputDisplay").textContent = data.morse; // Update UI
            }
        })
        .catch(error => console.error("Error fetching Morse input:", error));
}

// Call `fetchMorseInput` every 300ms for real-time updates
setInterval(fetchMorseInput, 300);
